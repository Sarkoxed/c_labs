...
def print_node(a, s, canvas, g):
    ...
    canvas.create_line(p[3] - s["wid"]//2, p[4] + s["heigh"]//2, p[3] + s["wid"]//2, p[4] + s["heigh"]//2, width=1, fill="black")
    canvas.create_text(p[3], p[4] + s["heigh"]//4 , text=str(a[0]), font="Verdana "+str(s["wid"] - s["wid"]//2), justify=CENTER, fill = "red")
    canvas.create_text(p[3], p[4] + 3*s["heigh"]//4 , text=str(a[3]), font="Verdana "+str(s["wid"] - s["wid"]//2), justify=CENTER, fill = "red")
...
sz = {"dep": s[0],
      "wid": int(sys.argv[1]),
      "heigh": int(sys.argv[1]),
      "tab_v": int(sys.argv[1])//4,
      "tab_g": int(sys.argv[1])//4,
      "col_max": 2**(s[0] - 1)
       }
